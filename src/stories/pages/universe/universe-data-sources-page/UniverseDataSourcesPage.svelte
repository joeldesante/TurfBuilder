...
<PageHeader
	title="Data Sources"
	subheading="Import people and location datasets into your organization's universe."
/>
...
<div class="grid gap-6 md:grid-cols-2">
	...
	<div class="rounded-xl border border-outline bg-surface-container p-6 flex flex-col gap-4">
		...
		{#if peopleResult}
			<div class="rounded-lg border border-outline bg-surface-container-low p-3 text-sm flex items-start gap-2" role="status" aria-live="polite">
				<CheckCircleIcon class="size-4 text-green-600 mt-0.5 shrink-0" />
				<div>
					<p class="font-medium text-on-surface">
						{peopleResult.imported.toLocaleString()} imported
						{#if peopleResult.skipped > 0}, {peopleResult.skipped.toLocaleString()} skipped{/if}
					</p>
		...
		{#if peopleError}
			<div class="rounded-lg border border-error/30 bg-error/5 p-3 text-sm flex items-start gap-2" role="alert">
		...
		<div class="mt-auto pt-2">
			<input bind:this={peopleFileInput} type="file" accept=".csv" class="sr-only" aria-label="Select people CSV file" onchange={handlePeopleFile} />
			<Button variant="outline" disabled={!onImportPeople} loading={peopleImporting} onclick={() => peopleFileInput?.click()}>
				<UploadSimpleIcon />
				Import CSV
			</Button>
		</div>
	...
	<div class="rounded-xl border border-outline bg-surface-container p-6 flex flex-col gap-4">
		...
		<p class="text-sm text-on-surface-subtle">
			...
			Include latitude and longitude to enable turf cutting.
		</p>
			...
			<div class="rounded-lg border border-outline bg-surface-container-low p-3 text-sm flex items-start gap-2" role="status" aria-live="polite">
				...
				<div>
					<p class="font-medium text-on-surface">
						...
						{#if locationsResult.skipped > 0}, {locationsResult.skipped.toLocaleString()} skipped{/if}
					</p>
		...
		<div class="mt-auto pt-2">
			<input bind:this={locationsFileInput} type="file" accept=".csv" class="sr-only" aria-label="Select locations CSV file" onchange={handleLocationsFile} />
			<Button variant="outline" disabled={!onImportLocations} loading={locationsImporting} onclick={() => locationsFileInput?.click()}>
				<UploadSimpleIcon />
				Import CSV
			</Button>
		</div>
