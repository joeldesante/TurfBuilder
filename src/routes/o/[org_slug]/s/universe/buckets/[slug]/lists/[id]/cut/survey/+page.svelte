<script lang="ts">
	...
	const org_slug = $page.params.org_slug!;
	...
	let storedListId = $state('');
	let storedBucketId = $state('');
	...
	onMount(() => {
		const stored = sessionStorage.getItem('universe_pending_turfs');
		if (!stored) {
			goto(cutHref);
			return;
		}
		...
		storedListId = parsed.listId ?? '';
		storedBucketId = parsed.bucketId ?? '';
	...
	async function save() {
		...
		const response = await fetch(`/o/${org_slug}/s/api/turf/create`, {
			...
			body: JSON.stringify({
				...
				list_id: storedListId,
				bucket_id: storedBucketId
			})
		});
		...
		if (response.ok) {
			sessionStorage.removeItem('universe_pending_turfs');
			await goto(listHref);
		}
	}
</script>
...
<PageHeader
	...
	breadcrumbs={[
		...
		{ label: 'List', href: listHref },
		{ label: 'Cut Turfs', href: cutHref },
		{ label: 'Set Survey' }
	]}
>
	{#snippet actions()}
		<Button variant="outline" href={cutHref}>← Back</Button>
