	...
	const { Story } = defineMeta({
		...
		argTypes: {
			type: {
				...
				description:
					'The type of input, which determines the expected format and may trigger specific virtual keyboards on mobile devices.'
			},
			placeholder: {
				control: 'text',
				description:
					'A short hint that demonstrates the expected format of the input when empty. This should only be used when needed.'
			},
			value: {
				control: 'text',
			...
			id: {
				...
				control: false,
				description:
					'The id attribute for the input element. Auto-inherited from `FormField` context if not provided.'
			},
			disabled: {
				description:
			...
			readonly: {
				description:
					'Whether the input is read-only. This prevents user input but does not disable the element.',
				control: 'boolean'
			}
		},
		parameters: {
			docs: {
				subtitle:
					'A styled native HTML Input element. To use with a label, helper text, validation, etc., nest within the FormField component.'
			}
		}
	});
</script>
